...
- **`mode: "sequential" | "standalone"`** — a single prop, decided once per page load from `status === "Brouillon"`, threaded `index → SectionContent → section → Shell`. `sequential` for Brouillon; `standalone` (today's behavior, unchanged) for Publiée (clean or Modifiée). It is **not** an overload of `hideActions`; the two are orthogonal and compose (a nothing-to-save notice section in sequential mode degrades its footer to plain "Suivant"). See the **Editing mode** glossary entry.
