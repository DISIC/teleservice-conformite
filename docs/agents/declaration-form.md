...
pages/dashboard/declarations/[id]/index.tsx     page entry: SSR, status → editing mode,
  ...
  └─ components/declaration/sections/Content.tsx   SECTION_DEFINITIONS slug → definition, mounts Section
       └─ sections/Section.tsx                     the one runtime: readOnly machine, autosave, error
            │                                      reveal, shell + <form>, terminal publish CTA, source radio
            ├─ items/{Infos,Audit,Schema,Contact}.tsx   one defineSection() per Section (Audit exports 4,
            │                                          one per Sub-section, via auditSubSection)
            ...
            ├─ domain/declaration/sectionEditing.ts  pure behaviour matrix: body mode, autosave, terminal
            └─ definition.useSave → api.<section>.…  tRPC mutation
                 └─ server/api/utils/section-write.ts  saveSection: merge rule → one write, status derived
```
...
| ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Which Section renders, prev/next navigation             | `domain/declaration/sections.ts` → `components/declaration/sections/Content.tsx` (`SECTION_DEFINITIONS`)                                                                                                                                                                                                                                                                                                                                |
...
| What a Section declares                                 | `components/declaration/sections/defineSection.ts` (`SectionDefinition`: slug, schema, `toValues`, `useSave`, `renderForm`, optional `isEditable` / `hideActions` / `autosaveWhen` / `library`)                                                                                                                                                                                                                                         |
| A Section's behaviour in a given mode                   | `domain/declaration/sectionEditing.ts` (`resolveSectionEditing`: body mode, custom edit, autosave, terminal, starts read-only) — pure, unit-tested                                                                                                                                                                                                                                                                                      |
| A Section's render skeleton, edit/save buttons          | `components/declaration/sections/Section.tsx` (the runtime) → `components/declaration/sections/Shell.tsx` (button matrix)                                                                                                                                                                                                                                                                                                               |
| Autosave                                                | `sections/Section.tsx` → `sections/hooks/useAutosave.ts` (debounce = `AUTOSAVE_DEBOUNCE_MS` in `forms/formOptions.ts`); enabled by `resolveSectionEditing` (sequential, custom edit, something to save) and the definition's `autosaveWhen`                                                                                                                                                                                             |
...
| Status / Declaration state (notice card)                | `domain/declaration/status.ts` (binary lifecycle) and `state.ts` (`getDeclarationState`) — distinct axes, see CONTEXT.md; the badge is `components/declaration/StatusBadge.tsx`, shared with the list tables via `components/declaration/tableColumns.tsx`                                                                                                                                                                              |
...
`items/Contact.tsx` is the canonical example: one `defineSection` call declaring the slug, the Zod schema, the `declarationTo<X>Values` mapper, a `useSave` hook binding `api.contact.upsert`, a `renderForm`, and a `library` block (kind, legend, radio options). It contains no other hook and no JSX beyond the fields. The four Audit Sub-sections are built by `auditSubSection` in `items/Audit.tsx`, which binds the shared `api.audit.update` and maps each form's values to its slice of the patch. Everything else — read-only state, autosave, error reveal, the shell, the publish CTA, the source radio — lives in `Section.tsx` and is decided by `resolveSectionEditing`.
...
5. `defineSection` in `components/declaration/sections/items/<Name>.tsx` — slug, schema, `toValues`, `useSave`, `renderForm`; add a `library` block if Library-sourced.
6. Register the slug in `SECTION_SLUGS` + `SECTIONS` (`sections.ts`) — title, badge predicates, validation.
7. Add the definition to `SECTION_DEFINITIONS` in `Content.tsx`.

## Gotchas

- `Content.tsx` keys `Section` by slug: navigating Sections always remounts, so edit state never survives navigation. This is also what keeps the runtime's hook order stable — a definition (and its `useSave` hook) never changes within a mount.
...
- The publish CTA on the terminal Section (the last of `SECTION_SLUGS`, in sequential mode) bypasses the form's own submit (`onPublish` on the shell). Terminality is derived from the slug order, never configured.
