...
| Source mode (Linked / Custom / Skipped) and the Library | `utils/declaration/sourceMode.ts` → `useSourceMode.ts` → `useLibraryLink.ts` → `components/declaration/sections/SourceModeSection.tsx`; mutations in `server/api/routers/library/` (`index.ts` routes → kind-parametric `service.ts`)                                                                                                                                                                                             |
...
| The save path server-side                               | `declarationProcedure` (`server/api/trpc.ts`, guard + `ctx.declaration` via `server/api/utils/declaration-access.ts`) → section router (`server/api/routers/library/index.ts` `contact.upsert`, `audit.ts` `update`, `declaration/index.ts` `update`) → `server/api/utils/publish-comparison.ts` (`recalculateDeclarationStatus`)                                                                                                 |
...
| The published snapshot and its public rendering         | `utils/declaration-content.ts` (`extractDeclarationContentToPublish`, `parsePublishedDeclaration` — the Zod contract) → `utils/declaration/publishedMarkdown.ts` (`buildPublishedMarkdown`, pure) → `components/declaration/PublishedTemplate.tsx` + `MarkdownToJsx.tsx`; public page `pages/declarations/[id]/publish.tsx` swaps the app header via `renderHeader` (`components/layout/pageHeader.ts`) for `PublishedHeader.tsx` |
...
4. Router with an `upsert` built on `declarationProcedure` (input `{ declarationId, values }`, read `ctx.declaration`) in `server/api/routers/<name>.ts`, wired into `server/api/root.ts`; end the mutation with `recalculateDeclarationStatus`.
