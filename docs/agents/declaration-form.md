...
| Source mode (Linked / Custom / Skipped) and the Library | `domain/declaration/sourceMode.ts` (pure derivation) → `sections/hooks/useSourceMode.ts` → `useLibraryLink.ts` (+ `sections/applyLibrarySection.ts`) → `components/declaration/sections/SourceModeSection.tsx`; mutations in `server/api/routers/library/` (`index.ts` routes → kind-parametric `service.ts`)                                                                                                                |
