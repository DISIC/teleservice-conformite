...
| Source mode (Linked / Custom / Skipped) and the Library | `utils/declaration/sourceMode.ts` → `useSourceMode.ts` → `useLibraryLink.ts` → `components/declaration/sections/SourceModeSection.tsx`; mutations in `server/api/routers/library.ts`        |
