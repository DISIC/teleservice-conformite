...
Le contenu RGAA 5 vit dans **`content/`** à la racine du dépôt, en markdown pur, édité par les contributeurs externes. Un paquet **`packages/content`** le lit, le valide (Zod) et produit les **Données publiées** (fichiers de données versionnés, nouveau format avec schéma JSON publié). La CI exécute la même validation sur toute PR touchant `content/`, avec des erreurs en français.
...
| Format des critères     | Un fichier YAML par critère                   | Dossier par critère, markdown à frontmatter, un fichier par test                           | Habitude des contributeurs, conflits de fusion, prose lisible ; le YAML subsiste dans le frontmatter (2–3 champs)                                                                           |
...
| Fichiers JSON générés   | Commités (hook pre-commit)                    | Générés au build et servis à une URL stable                                                | Plus d'artefacts en git ; le site devient le point de distribution                                                                                                                          |
| Format des données      | Reprise implicite du format RGAA 4            | Nouveau format, schéma JSON publié, versionné dans l'URL                                   | Le format RGAA 4 n'a pas de notion de Référentiel ; les outils doivent changer de toute façon                                                                                               |
...
**Décision.** Contenu dans **`content/`** à la racine, markdown pur, aucun code. Loader dans **`packages/content`**. CI sur `content/**` avec messages en français.
...
content/
  ...
  README.md               guide de contribution en français
```
...
- _`content/` sous `src/`._ `src/` est le code ; le contenu est une entrée du build, visible à la racine pour les contributeurs.
- _`content/` transformé en paquet._ Mettrait `package.json`, `src/` et `node_modules` sous les yeux des contributeurs ; le code va dans `packages/content`.
- _Un fichier YAML par critère_ (voir § 2 et l'annexe B).
- _Collections Payload._ Incompatible avec la contribution par PR en fichiers.

### 3.5 Données publiées

**Décision.** Nouveau format reflétant le modèle : un fichier par Référentiel (Critères applicables, Tests, références, cas particuliers, notes), un fichier « modèle complet » (tous les Critères avec leur applicabilité), un fichier glossaire (avec applicabilité par Terme). Versionnées dans l'URL (par exemple `/rgaa/data/5/web/criteres.json`), générées au build, jamais commitées sauf les Versions antérieures figées. Liens absolus produits depuis une URL de base configurée. **Schéma JSON publié** à côté des fichiers : c'est lui la promesse de compatibilité.

**Alternatives écartées.** Reprendre le format RGAA 4 (aucune place pour le Référentiel, bizarreries pérennisées) ; publier les deux formats (double surface à maintenir).
...
**Décision.** Documentation et gabarits : `content/README.md` en français (arborescence, champs de frontmatter, ouvrir une PR depuis le navigateur), dossier `content/_modeles/`, gabarit de PR (Référentiel et Critère touchés), formulaires d'issue (« erreur dans un critère ou un test », « proposition d'évolution », « glossaire »). Fichier de propriétaires de code : approbation d'un éditeur requise sur `content/`, d'un développeur ailleurs. Erreurs de CI en français avec fichier et ligne.
...
2. **`content/` et `packages/content`.** Schémas Zod, parsing, contrôles (messages en français), `thematiques.yml`, quelques Critères d'exemple migrés de 4.1.2 pour le réalisme ; job CI sur `content/**`.
...
5. **Données publiées.** Générateur, schéma JSON publié, service sous `/rgaa/data/5/…`, convention de dossier pour les Versions figées.
6. **Pagefind** avec filtre par Référentiel.
7. **Kit de contribution.** README, `_modeles/`, gabarit de PR, formulaires d'issue, propriétaires de code.
...
- Identifiants GitHub des éditeurs propriétaires de `content/`.
...
```
content/              markdown, contributeurs
apps/site/            Next.js App Router, export statique, domaine racine
apps/teleservice/     application actuelle, Pages Router, sous-domaine
packages/content/     loader : schémas, contrôles, génération des Données publiées, script `check` pour la CI
docs/                 documentation d'équipe (ce document, ADR)
CONTEXT.md            glossaire unique, deux groupes
```

## Annexe B — Pourquoi le YAML du cadrage a disparu

Le cadrage proposait un fichier YAML par critère pour sa lisibilité tabulaire et sa validation facile. La session a gardé le YAML là où il est bon (deux ou trois scalaires de frontmatter, `thematiques.yml`) et l'a retiré du reste :

- les contributeurs éditent depuis quatre ans du markdown à petit frontmatter ; un document entier en YAML est sensible à l'indentation et à la ponctuation, et les intitulés français regorgent de deux-points et de points d'interrogation qui cassent un scalaire non cité ;
- un fichier par critère couvrant trois Référentiels fait trois cents lignes : deux experts modifiant deux tests différents entrent en conflit, un fichier par test jamais ;
- une méthodologie est une liste numérotée avec du code et des liens, c'est de la prose : lisible en markdown, illisible dans un scalaire bloc YAML, et relisible dans le diff de GitHub ;
- la validation, raison d'être du YAML, n'est pas perdue : le frontmatter reste du YAML validé par Zod, et la CI contrôle en plus l'arborescence et les liens.
