...
| `NODE_ENV`           | may stay unset: Next sets `production` itself in `next build` and `next start`. When set to `production`, Clever Cloud runs `pnpm install --prod`, so everything `next build` needs (TypeScript, `@types/*`) is declared in the teleservice `dependencies`. `CC_NODE_DEV_DEPENDENCIES=install` does not work with pnpm (it runs `pnpm install --prod false`). |
