...
- **Sequential** — used while the Declaration is **Brouillon** (never published). The [[section]]s are chained into one guided walkthrough: every section renders permanently editable (no read-only toggle, no per-section Modifier/Annuler/Enregistrer). Fields **autosave** as they change — text on blur, checkbox/select/radio on change, arrays on add/remove/reorder + item-field blur — each through a lenient partial mutation that merges the single field (the audit-upsert convention, now shared by contact/schema). The footer is plain **"Précédent" / "Suivant"**: navigation flushes the focused field then **always proceeds** — it never validates or blocks. Section submit no longer runs the section schema. The final section (Contact) ends the walkthrough with the completeness gate ("Prévisualiser et publier"), which flushes and runs `validateDeclaration` directly — no per-section check first — surfacing a single, live, page-level error summary.

  **Validation is deferred to the gate, with one advisory layer.** Before a publish attempt, fields show **format-only** feedback live (onChange) — a malformed email/URL goes red, but emptiness never nags and nothing blocks. Required and cross-field rules (contact's email-or-url, the audit's realised-only fields) live only in the gate. After a publish attempt (the in-session "publish attempted" flag that gates the error summary — it resets on reload / page navigation), every mounted section additionally runs its **full** section schema inline, so required errors surface both in the summary and on the fields. Per field, one source at a time: format-only before the attempt, full-schema after. Completeness is owned entirely by the Zod gate — Payload's collection no longer enforces it (no `requiredWhenRealised`, no required `actionPlanUrls`), so an incomplete realised audit can persist mid-walkthrough.
