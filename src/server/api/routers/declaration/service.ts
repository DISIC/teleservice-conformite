...
import {
	...
	type importedDeclarationDataSchema,
...
/** Legacy client-side import create path. Superseded by the ARA / IA paths. */
export const createDeclarationFromUrl = async (
	payload: Payload,
	userId: number,
	input: z.infer<typeof importedDeclarationDataSchema>,
): Promise<number> => {
	const {
		service,
		taux,
	...
	try {
		const declarationName = await getDefaultDeclarationName(payload, userId);
